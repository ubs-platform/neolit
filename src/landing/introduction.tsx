...
export class Introduction extends NeolitComponent {
    ...
    apiCards = state([
        ...
        },
        {
            ...
            summary: "Yalnızca ilgili alt bloğu yeniden çizerek daha verimli günceller.",
        },
        {
            title: "For",
    ...
    render(): HTMLElement {
        ...
        return <div className="min-h-screen bg-gradient-to-br from-cyan-50 via-white to-orange-50 px-4 py-10 text-slate-800 sm:px-8">
            <main className="mx-auto flex w-full max-w-6xl flex-col gap-8">
                    ...
                    <div className="relative z-10 flex flex-col gap-6">
                        ...
                        <div className="space-y-3">
                            ...
                            <p className="max-w-3xl text-base leading-7 text-slate-600 sm:text-lg">
                                Bu sayfa, doğrudan package API'lerinden gelen yapı taşlarını ve test klasöründeki
                                gerçek örnek bileşenleri bir araya getirir. Minimal sınıf tabanlı bir yaklaşımla,
                                reaktif state ve yapısal render yardımcılarını tek noktada görebilirsin.
                            </p>
                        </div>

                        <div className="flex flex-wrap gap-3">
                            <span className="rounded-full bg-cyan-100 px-3 py-1 text-xs font-semibold text-cyan-900">@ubs-platform/neolit/core</span>
                            <span className="rounded-full bg-orange-100 px-3 py-1 text-xs font-semibold text-orange-900">@ubs-platform/neolit/structural</span>
                            <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">Tailwind v4</span>
                        </div>
                    </div>
                </section>
                ...
                <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    <For items={this.apiCards}>
                        {(card) => <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                            <h2 className="text-lg font-bold text-slate-900">{card.title}</h2>
                            <p className="mt-2 rounded-lg bg-slate-900/95 px-3 py-2 font-mono text-xs text-cyan-200">{card.signature}</p>
                            ...
                            <Stateful state={this.activeTab}>
                                {() => {
                                    ...
                                    return <>
                                        ...
                                        <button className={tabButtonClass("state")} onclick={() => this.switchTab("state")}>State</button>
                                        <button className={tabButtonClass("stateful")} onclick={() => this.switchTab("stateful")}>Stateful</button>
                                        <button className={tabButtonClass("for")} onclick={() => this.switchTab("for")}>For</button>
                                    </>
                                }}
                            </Stateful>
                    ...
                    <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-5">
                        <Stateful state={this.activeTab}>
                            {() => this.activeTab.get() === "state"
                                ? <div className="space-y-3">
                                    <p className="text-sm text-slate-600">Counter state'i güncelleniyor, ekranda anlık değeri görüyorsun.</p>
                                    <div className="flex items-center gap-3">
                                        <button className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white" onclick={() => this.incrementCounter()}>
                                            +1 artır
                                        </button>
                                        <Stateful state={this.counter}>
                                            {() => <strong className="text-xl text-cyan-700">{this.counter}</strong>}
                                        </Stateful>
                                    </div>
                                </div>
                                : this.activeTab.get() === "stateful"
                                    ? <div className="space-y-2">
                                        <p className="text-sm text-slate-600">Stateful yalnızca kendi children bloğunu tekrar render eder.</p>
                                        <p className="rounded-lg bg-white p-3 font-mono text-xs text-slate-700">{"<Stateful state={flag}>{() => flag.get() ? <A/> : <B/>}</Stateful>"}</p>
                                    </div>
                                    : <div className="space-y-2">
                                        <p className="text-sm text-slate-600">For ile dizi state'ini declarative şekilde listeleyebilirsin.</p>
                                        <p className="rounded-lg bg-white p-3 font-mono text-xs text-slate-700">{"<For items={items}>{(item) => <li>{item}</li>}</For>"}</p>
                                    </div>
                            }
                        </Stateful>
                    </div>

...
export class State<DATA> {
  ...
  unsubscribe(
    listener: null | undefined | ((newData: DATA, oldData?: DATA) => void),
  ): void {
    if (!listener) return;
  ...
  arrayMap<NEW_DATA>(
    mapper: (item: any, index: number) => NEW_DATA,
  ): State<NEW_DATA[]> {
...
export class ComputedState<DATA> extends State<DATA> {
  ...
  constructor(
    ...
    computeFn: (...stateValues: any[]) => DATA,
  ) {
    super(
      computeFn(
        statesListened.map((stateOrPlain) => getStateValue(stateOrPlain)),
      ),
    );
  ...
  recompute(): void {
    ...
    const newData = this.computeFn(
      this.statesListened.map((stateOrPlain) => getStateValue(stateOrPlain)),
    );
    const triggerFlag = this.determineTriggerIsRequired(newData);
...
export function computed<DATA>(
  statesListened: StateOrPlain<any>[],
  computeFn: (values: any[]) => DATA,
): ComputedState<DATA> {
...
export function isPlainObject<T>(value: any): value is T {
  return (
    typeof value === "object" && value !== null && !(value instanceof State)
  );
}

export function isTheyEqualArrays<T>(
  a: StateOrPlain<T>[],
  b: StateOrPlain<T>[],
): boolean {
...
export class AsyncState<T> extends State<T> {
  ...
  private _setAsync(promise: Promise<T>): Promise<void> {
    if (this.busy.get()) {
      console.warn(
        "AsyncState is already busy with another promise. Overwriting with a new promise.",
      );
    }
  ...
  public allInComputed(): ComputedState<{
    data: T;
    busy: boolean;
    error: Error | null;
  }> {
